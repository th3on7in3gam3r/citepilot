...
export type SignalDeskPublishBody = {
  ...
  meta: {
    ...
    byline: string;
    topics: string[];
    /** Marks the post as CitePilot-sourced for Signal Desk soft-gate attribution. */
    citepilot: true;
    source: "citepilot";
  };
};

export const CITEPILOT_ATTRIBUTION_HREF =
  "https://getcitepilot.com/?utm_source=signaldesk&utm_medium=attribution&utm_campaign=post_footer";

export const CITEPILOT_ATTRIBUTION_HTML = `<p class="citepilot-attribution" data-citepilot-attribution="1">Generated by <a href="${CITEPILOT_ATTRIBUTION_HREF}">CitePilot</a></p>`;

/** Append CitePilot attribution footer if missing (idempotent). */
export function withCitePilotAttributionHtml(html: string): string {
  if (/data-citepilot-attribution\s*=\s*["']?1["']?/i.test(html)) {
    return html;
  }
  const trimmed = html.trimEnd();
  return `${trimmed}\n${CITEPILOT_ATTRIBUTION_HTML}`;
}
...
export function buildSignalDeskPublishBody(input: {
  ...
  return {
    title: input.title,
    slug: input.slug,
    content: withCitePilotAttributionHtml(input.html),
    excerpt,
