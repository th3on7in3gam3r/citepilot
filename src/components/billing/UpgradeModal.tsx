...
export function UpgradeModal({
  ...
  return (
    <dialog
      ref={dialogRef}
      className="fixed inset-0 z-[80] m-0 h-full max-h-none w-full max-w-none border-0 bg-transparent p-0 open:bg-black/50"
      aria-labelledby="upgrade-modal-title"
      onClose={onDismiss}
    >
      {/* Centering lives on the inner wrapper — never put display:flex on <dialog>
          or it overrides the UA rule dialog:not([open]) { display: none }. */}
